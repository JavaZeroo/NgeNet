...
def main(args):
    setup_seed(22)
    config = decode_config(os.path.join(CUR, 'configs', 'kitti.yaml'))
    config = edict(config)
    config.architecture = architectures[config.dataset]
    config.num_workers = 4
    test_dataset = Kitti(root=args.data_root, 
                         split='test', 
                         aug=False, 
    ...
    Ts, gt_Ts = np.array(Ts), np.array(gt_Ts)
    ...
    rot_threshold = 5
    trans_threshold = 2
    rot_flag = rot_error < rot_threshold
    trans_flag = trans_error < trans_threshold

    recall = (rot_flag & trans_flag).sum() / len(rot_flag)
    RRE = np.mean(rot_error[rot_flag])
    RTE = np.mean(trans_error[trans_flag])

    print('Recall: ', fmat(recall))
    print('RRE: ', fmat(RRE))
    print('RTE: ', fmat(RTE))
